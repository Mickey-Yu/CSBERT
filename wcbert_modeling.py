...
class BertLayer(nn.Module):
    ...
    def __init__(self, config, fusion_method, has_word_attn=False):
        super().__init__()
        ...
        self.lstm = nn.LSTM(input_size=config.hidden_size, hidden_size=config.hidden_size // 2, batch_first=True,
                            bidirectional=True,
                            dropout=config.hidden_dropout_prob, num_layers=2)
        self.lstm_transform = nn.Linear(config.hidden_size, config.hidden_size)
    ...
    def forward(
        ...
        if self.has_word_attn:
            ...
            word_outputs = self.dropout(word_outputs)  # word_outputs: [4,256,5,768]
            
            # layer_output:[4, 256, 768], word_outputs: [4,256,5,768]
            
            # fusion_method select in [no_words, add, cosi, attention, sharp_attention, gumbel_softmax ]
            if self.fusion_method == 'no_words':  # 0.do not fuse any word
                layer_output = layer_output  # layer_output:[4, 256, 768]

            if self.fusion_method == 'add': # 1.直接相加
                layer_output = layer_output.unsqueeze(2)  # layer_output: [2,256,1,768]
                layer_output = layer_output + word_outputs
                layer_output = layer_output.mean(2)  # layer_output:[4, 256, 768]
            
            if self.fusion_method == 'attention':
                # attention_output = attention_output.unsqueeze(2) # [N, L, D] -> [N, L, 1, D]
                alpha = torch.matmul(layer_output.unsqueeze(2), self.attn_W)  # [N, L, 1, D]    alpha: [4,256,1,768]
                # alpha: [4,256,1,768], word_outputs: [4,256,5,768] -> [4,256,1, 5]
                alpha = torch.matmul(alpha, torch.transpose(word_outputs, 2, 3))  # [N, L, 1, W]
                alpha = alpha.squeeze()  # [N, L, W]    alpha: [4, 256, 1, 5] -> alpha: [4, 256, 5]
                alpha = alpha + (1 - input_word_mask.float()) * (-10000.0)  # alpha: [4, 256, 5]
                alpha = torch.nn.Softmax(dim=-1)(alpha)  # [N, L, W]  # alpha: [4, 256, 5]
                alpha = alpha.unsqueeze(-1)  # [N, L, W, 1] alpha: [4, 256, 5, 1]
                weighted_word_embedding = torch.sum(word_outputs * alpha, dim=2)  # word_outputs: [4,256,5,768] * alpha:[4, 256, 5, 1] = [4,256,5,768]->[4,256,768]
                layer_output = layer_output + weighted_word_embedding # layer_output:[4, 256, 768], weighted_word_embedding:[4,256,768]
                
            if self.fusion_method == 'cosi':
                penalty = torch.matmul(layer_output.unsqueeze(2), self.attn_W) # penalty: [2,256,1,768]
                penalty = F.cosine_similarity(word_outputs, penalty,dim=-1) # penalty : [2, 256, 5]
                penalty = penalty.unsqueeze(-1) # penalty : [2, 256, 5, 1]

                # word_outputs:[2, 256, 5, 768], penalty : [2, 256, 5, 1]
                # penalty = self.dropout(penalty)
                weighted_word_embedding = torch.sum(word_outputs * penalty, dim=2) # weighted_word_embedding:[2, 256, 768]
                layer_output = layer_output + weighted_word_embedding # layer_output:[2, 256, 768]
                '''
                # layer_output = self.dropout(layer_output)
                # layer_output = self.fuse_layernorm(layer_output)
                lstm_out, _ = self.lstm(layer_output)  # lstm_out: [2, 256, 768]
                lstm_out = self.dropout(lstm_out)
                lstm_out = self.lstm_transform(lstm_out)
                
                layer_output= layer_output + lstm_out  # layer_output:[2, 256, 768]
                '''
                
            if self.fusion_method == 'sharp_attention':  # 4.锐化的注意力机制
                alpha = torch.matmul(layer_output.unsqueeze(2), self.attn_W)  # alpha:[2, 256, 1, 768]

                alpha = torch.matmul(alpha, torch.transpose(word_outputs, 2, 3))  # alpha:[2, 256, 1, 768] x [2,256,768,5] = [2,256,1,768]字词相乘
                alpha = alpha.squeeze()  # [2,256,768]
                alpha = alpha + (1 - input_word_mask.float()) * (-10000.0)
                # alpha = torch.tan(alpha)
            ...
            if self.fusion_method == 'gumbel_softmax':  # 5.gumbel_softmax
                alpha = torch.matmul(layer_output.unsqueeze(2), self.attn_W)  # alpha:[2, 256, 1, 768]

                alpha = torch.matmul(alpha, torch.transpose(word_outputs, 2,
                                                            3))  # alpha:[2, 256, 1, 768] x [2,256,768,5] = [2,256,1,768]字词相乘
                alpha = alpha.squeeze()  # [2,256,768]
                alpha = alpha + (1 - input_word_mask.float()) * (-10000.0)
                alpha = F.gumbel_softmax(alpha,tau=1,hard=True)  # [N, L, W]
                alpha = alpha.unsqueeze(-1)  # [N, L, W, 1]
                weighted_word_embedding = torch.sum(word_outputs * alpha, dim=2)  # [N, L, D]
                layer_output = layer_output + weighted_word_embedding
...
class BertEncoder(nn.Module):
    def __init__(self, config,fusion_method):
        super().__init__()
        self.config = config
        ...
        for i in range(config.num_hidden_layers):
            if i in self.add_layers:
                total_layers.append(BertLayer(config, fusion_method, True))
            else:
                total_layers.append(BertLayer(config, fusion_method, False))
...
class WCBertModel(BertPreTrainedModel):
    def __init__(self, config, fusion_method, add_pooling_layer=True):
        super(WCBertModel, self).__init__(config)

        self.embeddings = BertEmbeddings(config)
        self.encoder = BertEncoder(config,fusion_method)
    ...
    def forward(
        ...
        '''
        # cosine-similaraty
        # matched_word_embeddings: [8, 256, 5, 200]
        word_outputs = self.word_transform(matched_word_embeddings)  # [N, L, W, D] word_outputs:[8, 256, 5, 768]
        word_outputs = self.act(word_outputs)
        word_outputs = self.word_word_weight(word_outputs)
        word_outputs = self.dropout(word_outputs)  # 词向量 word_outputs:[8, 256, 5, 768]
        # print('word_outputs shape:',word_outputs.shape)
        penalty = torch.matmul(embedding_output.unsqueeze(2), self.attn_W)  # penalty: [8,256,1,768]
        penalty = F.cosine_similarity(word_outputs, penalty, dim=-1)  # penalty : [8, 256, 5]
        penalty = penalty.unsqueeze(-1)  # penalty : [2, 256, 5, 1]

        # word_outputs:[2, 256, 5, 768], penalty : [2, 256, 5, 1]
        # penalty = self.dropout(penalty)
        weighted_word_embedding = torch.sum(word_outputs * penalty, dim=2)  # weighted_word_embedding:[2, 256, 768]
        #weighted_word_embedding = self.dropout(weighted_word_embedding)
        #weighted_word_embedding = self.lstm_transform(weighted_word_embedding)
        embedding_output = embedding_output + weighted_word_embedding  # layer_output:[2, 256, 768]
        # lstm_out, _ = self.lstm(embedding_output)  # lstm_out: [2, 256, 768]
        # lstm_out = self.dropout(lstm_out)
        # lstm_out = self.lstm_transform(lstm_out)
        
        #embedding_output= embedding_output + lstm_out  # layer_output:[2, 256, 768]

        #embedding_output = self.dropout(embedding_output)
        #embedding_output = self.fuse_layernorm(embedding_output)
        '''
...
class WCBertCRFForTokenClassification(BertPreTrainedModel):
    def __init__(self, config, args, pretrained_embeddings, num_labels):
        ...
        self.bert = WCBertModel(config=config,fusion_method=args.fusion_method)
