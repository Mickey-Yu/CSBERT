CUDA_VISIBLE_DEVICES=0 python3 -m torch.distributed.launch --master_port 13117 --nproc_per_node=1 \
       Trainer.py --do_train --do_eval --do_predict --evaluate_during_training \
                  --data_dir="data/dataset/POS/ctb6" \
                  --output_dir="data/result/POS/ctb6/WCBertCRF_Token_attention" \
				  --logging_dir="data/log/POS/ctb6/WCBertCRF_Token_attention" \
                  --label_file="data/dataset/POS/ctb6/labels.txt" \
                  --saved_embedding_dir="data/dataset/POS/ctb6" \
