# -*- coding: utf-8 -*-
# @Time    : 2021/11/30 12:47
# @Author  : mickey yu
# @File    : Trainer.py
"""
The training of CSBERT for Chinese Sequence Labelling
"""
...
def get_dataloader(dataset, args, mode='train'):
    ...
    if args.local_rank != -1:
        ...
        if mode == 'train':
            ...
            sampler = SequentialSampler(dataset)
    if mode == 'train':
        batch_size = args.per_gpu_train_batch_size  # =4
    else:
        batch_size = args.per_gpu_eval_batch_size   # =16
...
def train(model, args, train_dataset, dev_dataset, test_dataset, label_vocab, tb_writer, model_path=None):
    ...
    if args.max_steps > 0: # max_steps=-1 不执行
    ...
    if args.fp16 and _use_apex:  # fp16=False  不执行
    ...
    if args.local_rank != -1:  # args.local_rank=0  不执行
        model = model.cuda()
    ...
    if args.local_rank == 0 or args.local_rank == -1:    # args.local_rank=0
        ...
        logger.info("  fusion_method = %s", args.fusion_method)

    global_step = 0
    epoch = 0
    epochs_trained = 0
    steps_trained_in_current_epoch = 0
    if model_path is not None: # load checkpoint and continue training  # model_path=None 不执行
        try:
    ...
    for epoch in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            if steps_trained_in_current_epoch > 0:  # 一开始steps_trained_in_current_epoch=0  不执行
            ...
            if args.fp16 and _use_native_amp:  # 不执行
                with autocast():
            ...
            if args.n_gpu > 1:  # args.n_gpu=1  不执行
                loss = loss.mean()
            if args.gradient_accumulation_steps > 1:  # gradient_accumulation_steps=1  不执行
                loss = loss / args.gradient_accumulation_steps

            if args.fp16 and _use_native_amp:  # 不执行
                scaler.scale(loss).backward()
            elif args.fp16 and _use_apex:  # 不执行
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0 or \
                ...
                if args.fp16 and _use_native_amp:  # 不执行
                    ...
                    torch.nn.utils.clip_grad_norm_(model.parameters(), args.max_grad_norm)  # max_grad_norm=1.0
                elif args.fp16 and _use_apex:  # 不执行
                ...
                ## logger and evaluate
                if (args.logging_steps > 0 and global_step % args.logging_steps == 0):  # logging_steps=100
                    logs = {}
    ...
    logger.info('\nf1_scores = {}\n'.format(f1_scores))
    logger.info('Max f1-score : {}\n'.format(max(f1_scores.values())))
    logger.info('Key of Max f1-score: {} \n'.format(max(f1_scores, key=f1_scores.get)))
...
def evaluate(model, args, dataset, label_vocab, global_step, description="dev", write_file=False):
    ...
    for batch in tqdm(dataloader, desc=description):
        ...
        input_ids = batch[0].detach().cpu().numpy()  # detach的作用是断开计算图的梯度传递
    ...
    ## calculate metrics
    acc, p, r, f1, all_true_labels, all_pred_labels = seq_f1_with_mask(all_label_ids, all_predict_ids, all_attention_mask, label_vocab)
    metrics = {}
    metrics['acc'] = acc
    metrics['p'] = p
    metrics['r'] = r
...
def main():
    ...
    ## 1.prepare data
    # a. lexicon tree
    lexicon_tree = build_lexicon_tree_from_vocabs([args.word_vocab_file], scan_nums=[args.max_scan_num])  # vocab_files="data/vocab/tencent_vocab.txt", scan_nums=1000000
    ...
    if "msra" in args.data_dir: # 具体指出层次到weibo、msra等这种级别
    ...
    matched_words = get_corpus_matched_word_from_lexicon_tree(data_files, embed_lexicon_tree)  # 函数返回结果
    word_vocab = ItemVocabArray(items_array=matched_words, is_word=True, has_default=False, unk_num=5) # 实例化的对象
    label_vocab = ItemVocabFile(files=[args.label_file], is_word=False)  # 实例化的对象
    tokenizer = BertTokenizer.from_pretrained(args.vocab_file)  # vocab_file="data/berts/bert/vocab.txt"   实例化的对象
    ...
    if args.do_train:
        train_dataset = TaskDataset(train_data_file, params=dataset_params, do_shuffle=args.do_shuffle) # 实例化的对象
        dev_dataset = TaskDataset(dev_data_file, params=dataset_params, do_shuffle=False)  # 实例化的对象
        test_dataset = TaskDataset(test_data_file, params=dataset_params, do_shuffle=False)  # 实例化的对象
